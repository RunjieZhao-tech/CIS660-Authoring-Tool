...
//        get_column(lp, i, colValue);
//        for (int j = 1;j < quads.size() + 1;j++) {
//            if (colValue[j] == 0)continue;//not tiled face
//            Face* tiledQuad = quads[j - 1];//because we store quad id begins with 1
...
//	return result;
    ...
    int totalColNum = get_Ncolumns(lp);
    REAL* rowVal = nullptr;
    rowVal = (REAL*)malloc((totalColNum+1)*sizeof(*rowVal));
    REAL* colValue = nullptr;
    colValue = (REAL*)malloc((1 + quads.size() + tiles.size()) * sizeof(*colValue));//actually useless since it's a binary value
    ...
    std::vector<std::vector<Face*>> result;
    //find valid tile choice convert it to vector<Face*> used in result
    for (int i = 0;i < totalColNum;i++) {//weird, but here in row[0] is actually the value for value 1
